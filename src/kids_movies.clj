(ns kids-movies
  (:require [template :refer [template-page]]
            [movies-statistics :refer [search-by-genre search-by-country search-by-runtime
                                       ...
                                       kids-movies]]
            [popularity-prediction :refer [search-by-platform]]
...
(defn- movie-search-box
  ...
  []
  [:div.movie
   [:h2 "Search kids movies on Netflix, Hulu, Disney+ and Prime Video:"]
   (form-to [:post "/kids-movies"]
            [:table
             [:tr
...
(defn- pagination
  ...
  (if-not (= 0 last)
    [:p
     (if-not (= 1 page)
       [:span
        [:a {:href (str "/kids-movies/" criteria "&1")} "<< First"] " "
        (if-not (= 2 page)
          [:a {:href (str "/kids-movies/" criteria "&" (- page 1))} "< Previous"])])
     (if-not (= 1 last)
       [:span " " [:b (str page " of " last " pages")] " "])
     (if-not (= last page)
       [:span
        [:a {:href (str "/kids-movies/" criteria "&" (+ page 1))} "Next >"] " "
        (if-not (= (- last 1) page)
          [:a {:href (str "/kids-movies/" criteria "&" last)} "Last >>"])])]))
...
(defn- get-data-by-search-criteria
  ...
  [criteria]
  (cond
    (not-empty (search-by-year criteria (kids-movies))) (search-by-year criteria (kids-movies))
    (not-empty (search-by-title criteria (kids-movies))) (search-by-title criteria (kids-movies))
    (not-empty (search-by-country criteria (kids-movies))) (search-by-country criteria (kids-movies))
    (not-empty (search-by-genre criteria (kids-movies))) (search-by-genre criteria (kids-movies))
    (not-empty (search-by-platform criteria (kids-movies))) (search-by-platform criteria (kids-movies))
    (not-empty (search-by-language criteria (kids-movies))) (search-by-language criteria (kids-movies))
    (not-empty (search-by-imdb criteria (kids-movies))) (search-by-imdb criteria (kids-movies))
    (not-empty (search-by-runtime criteria (kids-movies))) (search-by-runtime criteria (kids-movies))
    :else nil))

(defn kids-movies-page
  "Show kids movies page depending on search criteria."
  ([uri] (template-page
          "Kids movies"
          uri
          (movies-layout (kids-movies) "all" 1)))
