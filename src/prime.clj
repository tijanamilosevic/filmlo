...
(defn- pagination
  ...
  (if-not (= 0 last) 
    [:p
     (if-not (= 1 page)
       [:span
        [:a {:href (str "/movies/prime/" criteria "&1")} "<< First"] " "
        (if-not (= 2 page)
          [:a {:href (str "/movies/prime/" criteria "&" (- page 1))} "< Previous"])])  
     (if-not (= 1 last)
       [:span " " [:b (str page " of " last " pages")] " "])    
     (if-not (= last page)
       [:span
        [:a {:href (str "/movies/prime/" criteria "&" (+ page 1))} "Next >"] " "
        (if-not (= (- last 1) page)
          [:a {:href (str "/movies/prime/" criteria "&" last)} "Last >>"])])]))  
...
(defn- get-data-by-search-criteria 
  ...
  (cond 
    ...
    (not-empty (search-by-language criteria movies-prime)) (search-by-language criteria movies-prime)
...
(defn prime-page
  "Show Movies Prime Video page depending on search criteria." 
  ([uri] (template-page 
           "Movies Prime Video" 
           uri 
           (movies-layout movies-prime "all" 1)))
