(ns movies-statistics
 (:require [clojure.data.json :as json]
           ...
           [popularity-prediction :refer [platform-name insert-platform]]))
...
(defn kids-movies-no-platform
  "Returns kids movies (age is 7+)"
  []
   (filter #(= (:age %) "7+")
           movies))

 (defn kids-movies
   "Returns kids movies with new key :platform and its value."
   []
   (map #(insert-platform % (platform-name
                             (:netflix %)
                             (:hulu %)
                             (:disney+ %)))
        (kids-movies-no-platform)))
